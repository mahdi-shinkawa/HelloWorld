...
namespace TCPIPClient
{
    public partial class frmClient : Form
    {
        ...
        private void buttonSendDataToServer_Click(object sender, EventArgs e)
        {
            ...
            //if (Tx_Txt.Text.Length <= xdata.szStringDataA.Length)
            //{
            //    Tx_Txt.Text.CopyTo(0, xdata.szStringDataA, 0, Tx_Txt.Text.Length);
            //    chunkSize = Tx_Txt.Text.Length;
            //}
            //else
            //    Tx_Txt.Text.CopyTo(0, xdata.szStringDataA, 0, xdata.szStringDataA.Length);
            ...
            //while (true)
            //{
            //    int PosFromEnd = Tx_Txt.Text.Length - pos;
            ...
            //        chunkSize = Tx_Txt.Text.Length - pos;
            //    else
            //        chunkSize = xdata.szStringDataA.Length;

            //    Tx_Txt.Text.CopyTo(pos, xdata.szStringDataA, 0, chunkSize);
            //    xdata.Data1 = (UInt32)chunkSize;
            ...
            string Tx_Buffer_str = string.Format("Client_Cpt = {0}", Client_Cpt++);
            byte[] Tx_Buffer = Encoding.ASCII.GetBytes(Tx_Buffer_str);
            SendMessageToServer(Tx_Buffer);


            if (client.Tx_Buffer != null)
            { 
                string Str = Encoding.UTF8.GetString(client.Tx_Buffer, 0, client.Tx_Buffer_Size); 
        ...
        public void PackReceived(byte[] message, int messageSize)
        {
            if (InvokeRequired)
            {
                this.Invoke(new ReceiveDataCallback(PackReceived), new object[] { message, messageSize });
        ...
        private void Clear_Tx_Btn_Click(object sender, EventArgs e)
        {
            Tx_Txt.Clear();
        }

        private void Clear_Rx_Btn_Click(object sender, EventArgs e)
        {
            Rx_Txt.Clear();
        }

        long ClientCpt = 0;
        private void Move_Motor_Btn_Click(object sender, EventArgs e)
        {
            
            //string Tx_Buffer_str = string.Format("\"Target Position\":{0}, \"Target Speed\":{1}\r\n", double.Parse(Target_Position_Txt.Text), double.Parse(Target_Speed_Txt.Text));
            string Tx_Buffer_str = string.Format("C,{0},{1},{2}\r\n", ClientCpt++, double.Parse(Target_Position_Txt.Text), double.Parse(Target_Speed_Txt.Text));

            byte[] Tx_Buffer = Encoding.ASCII.GetBytes(Tx_Buffer_str);
            SendMessageToServer(Tx_Buffer);
            Tx_Txt.Text += Tx_Buffer_str;
