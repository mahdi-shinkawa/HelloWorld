...
namespace TCPClient
{
    public class Client
    {
        ...
        public byte[] Tx_Buffer = null;
        public int Tx_Buffer_Size = 0;

        /// byte[] Tx_Buffer = null;<summary>
        ...
        private void OnDataReceived(IAsyncResult asyn)
        {
            try
            {
                ...
                Tx_Buffer = new byte[dataSize+1];
                Tx_Buffer_Size = dataSize;
                for (int i = 0; i < dataSize; i++)
                { 
                    Tx_Buffer[i] = socketData.DataBuffer[i];
                }
